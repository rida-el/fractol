...
int ft_map(t_data *mlx, double xmin, double xmax)
{
	double x,y;

	x = mlx->xmin + (mlx->juliax * ((xmax - xmin) / 800));
	y = mlx->ymin + (mlx->juliay * ((ymax - ymin) / 800));
}
...
int mandelbrot(double x, double y, t_data *mlx)
{
	...
	if (mlx->julia)
	{
			cx = ft_map(mlx->juliax, -0.5, 0.4);
			cy = ft_map(mlx->juliay, -0.5, 0.4);
	}
	while (it < 100)
	{
		temp = (x * x - y * y) + cx;
